...
class Theme
{
    ...
    public function __construct(array $config = array()) {
        ...
        $this->set('lang_abbr', $this->CI->router->user_lang);
    ...
    public function theme_path($uri = '') {
        return realpath(SERVERROOT . DS . 'themes' . DS . $this->theme . DS . $uri);
    }
    ...
    public function upload_path($uri = '') {
        return realpath(WEBROOT . DS . 'uploads' . DS . $uri);
    }
    ...
    /**
     * Changes the folder to 'assets' folder.
     * @access    public
     *
     * @param    string $uri path to file
     *
    ...
    /**
     * Returns the real path to the assets folder.
     * @access    public
     *
     * @param    string $uri file name.
     *
     * @return    string    the path if found, else FALSE.
     */
    public function assets_path($uri = '') {
        return realpath(WEBROOT . "assets/{$uri}");
    }
    ...
    protected function _load_file($type = 'view', $view = '', $data = array(), $return = false) {
        switch ($type) {
            ...
            case 'views':
                ...
                // prepare all path
                $paths = array(
                    build_path(SERVERROOT, 'themes', $this->theme, 'views', '_modules', $this->module),
                    build_path(SERVERROOT, 'themes', $this->theme, 'views'),
                ...
                if (!empty($paths)) {
                    ...
                    if ($found !== true) {
                        show_error("The requested view file '{$view}' could not be found in any of these folders: <ul><li>" .
                                   implode("</li><li>", array_unique($paths)) . "</li></ul>");
                    }
            ...
            case 'partials':
                // prepare all path
                $paths = array(
                    build_path(SERVERROOT, 'themes', $this->theme, 'views', '_modules', $this->module, '_partials'),
                    build_path(SERVERROOT, 'themes', $this->theme, 'views', '_partials'),
                ...
                if (!empty($paths)) {
                    ...
                    if ($found !== true) {
                        show_error("The requested partial file '{$view}' could not be found in any of these folders: <ul><li>" .
                                   implode("</li><li>", array_unique($paths)) . "</li></ul>");
                    }
            ...
            case 'layouts':
                ...
                // prepare all path
                $paths = array(
                    build_path(SERVERROOT, 'themes', $this->theme, 'views', '_modules', $this->module, '_layouts'),
                    build_path(SERVERROOT, 'themes', $this->theme, 'views', '_layouts'),
                ...
                if (!empty($paths)) {
                    ...
                    if ($found !== true) {
                        show_error("The requested layout file '{$view}' could not be found in any of these folders: <ul><li>" .
                                   implode("</li><li>", array_unique($paths)) . "</li></ul>");
                    }
            ...
            default:
                // prepare all path
                $paths = array(
                    build_path(SERVERROOT, 'themes', $this->theme, 'views', '_modules', $this->module, '_master'),
                    build_path(SERVERROOT, 'themes', $this->theme, 'views', '_master'),
                ...
                if (!empty($paths)) {
                    ...
                    if ($found !== true) {
                        show_error("The requested master view '{$view}' could not be found in any of these folders: <ul><li>" .
                                   implode("</li><li>", array_unique($paths)) . "</li></ul>");
                    }
     ...
     * @param    array $options associative array of options to apply first
     * @return mixed
     ...
     * @param    array $options associative array of options to apply first
     *
     * @return   mixed $return    whether to output or simply build
     *                           NOTE: you can pass $options instead of $title like so:
    ...
    public function load($view, $data = array(), $return = false) {
        ...
        $output = $this->_build_theme_output($view, $data);
     ...
     * @param   null|string $content value or null if $name is array
     * @param void|string $type
     * @param void|array $attrs
     *
     * @return  string
     ...
     * @param   mixed  $attrs attributes to append to string
     * @param  mixed $folder
     *
     * @return  string
     */
    public function css($file, $cdn = null, $attrs = '', $folder = null) {
        ...
        if ($file) {
            ...
            return '<link rel="stylesheet" type="text/css" href="' . $this->css_url($file, $folder) . '"' .
                _stringify_attributes($attrs) . '>' . "\n";
        }
    ...
    public function js($file, $cdn = null, $attrs = '', $folder = null) {
        ...
        if ($file) {
            // Use the 2nd parameter if it's set & the CDN use is enabled.
            ($this->cdn_enabled && $cdn !== null) && $file = $cdn;
            return '<script type="text/javascript" src="' . $this->js_url($file, $folder) . '"' .
                _stringify_attributes($attrs) . '></script>' . "\n";
        }
        return null;
    }
...
if (!function_exists('img_alt')) {
     ...
     * @param int $width
     * @param void|int $height
     * @param void|string $text
     * @param void|string $background
     * @param void|string $foreground
     *
     * @return  string
     */
    function img_alt($width, $height = null, $text = null, $background = null, $foreground = null) {
        ...
        return '<img src="//placehold.it/' . $params['width'] . 'x' . $params['height'] . '/' .
            $params['background'] . '/' . $params['foreground'] . '&text=' . $params['text'] . '" alt="Placeholder">';
    }
}
...
if (!function_exists('print_alert')) {
    ...
    function print_alert($message = '', $type = 'info', $view = 'alert') {
        ...
        return get_instance()->theme->partial($view,
                                              array(
                                                  'type'    => $type,
                                                  'message' => $message
                                              ),
