...
class Ion_auth_model extends CI_Model
{
    ...
    public function user($id = NULL) {
        ...
        $this->db->join('user_meta', 'user_meta.id = user.id', 'left');
        ...
        if (isset($groups)) {
            ...
            if (!empty($group_names)) {
                $this->db->join($this->tables['groups'],
                                $this->tables['users_groups'] . '.' . $this->join['groups'] . ' = ' .
    ...
    public function salt() {
        ...
        $salt = substr($salt, 0, $raw_salt_len);
    ...
    public function change_password($identity, $old, $new) {
        ...
        $old_password_matches = password_verify($old, $user->password);
    ...
    public function forgotten_password($identity) {
        ...
        if ($key != '' && $this->config->item('permitted_uri_chars') != '' &&
    ...
    public function forgotten_password_complete($code, $salt = FALSE) {
        ...
        if ($profile) {
            ...
            if ($this->config->item('forgot_password_expiration', 'ion_auth') > 0) {
                ...
                if (date('%Y-%m-%d %H:%i:%s', time()) - $profile->forgotten_password_time > $expiration) {
                    //it has expired
    ...
    public function get_attempts_num($identity, $ip_address = NULL) {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            ...
            $time = date('Y-m-d H:i:s', (time() - $this->config->item('lockout_time', 'ion_auth')));
            $this->db->where('time >', "'".$time."'", FALSE);
    ...
    public function update_last_login($id) {
        ...
        $this->db->update(
            $this->tables['users'],
            array('last_login' => date('Y-m-d H:i:s', time())),
            array('id' => $id)
        );
    ...
    public function clear_login_attempts($identity, $old_attempts_expire_period = 86400, $ip_address = NULL) {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            ...
            $old_attempts_expire_period =
                max($old_attempts_expire_period, $this->config->item('lockout_time', 'ion_auth'));
            ...
            $time = date('Y-m-d H:i:s', (time() - $old_attempts_expire_period));
            $this->db->or_where('time <', "'".$time."'", FALSE);
    ...
    public function increase_login_attempts($identity) {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            $data = array('ip_address' => '', 'login' => $identity, 'time' => (date('%Y-%m-%d %H:%i:%s', time())));
            if ($this->config->item('track_login_ip_address', 'ion_auth')) {
     ...
     * @author    Mathew
     */
    public function google_login($identity, $remember = true) {
        $this->trigger_events('pre_login');
    ...
    public function recheck_session() {
        $recheck = (NULL !== $this->config->item('recheck_timer', 'ion_auth')) ?
            $this->config->item('recheck_timer', 'ion_auth') : 0;

        if ($recheck !== 0) {
            $last_login = $this->session->userdata('last_check');
            if ($last_login + $recheck < time()) {
                $query = $this->db->select('id')
                                  ->where(array($this->identity_column => $this->session->userdata('identity'),
                                                'active'               => '1'
                                          ))
                                  ->limit(1)
                                  ->order_by('id', 'desc')
                                  ->get($this->tables['users']);
                if ($query->num_rows() === 1) {
                    $this->session->set_userdata('last_check', date('%Y-%m-%d %H:%i:%s', time()));
                } else {
    ...
    public function get_users_groups($id = FALSE) {
        ...
        return $this->db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' .
                                 $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description')
     ...
     * @return bool
     */
    public function set_meta($id, $data = array()) {
        $u = $this->db->get_where('user_meta', array('id' => $id), 1)->row();
        if ($u) {
            return $this->db->update('user_meta', $data, array('id' => $id));
        } else {
            $data['id'] = $id;
    ...
    public function get_meta($id, $data = array()) {
        ...
        $query = $this->db->get_where('user_meta', array('id' => $id));
        return $query->row();
    }
    ...
    public function remove_from_group($group_ids = FALSE, $user_id = FALSE) {
        ...
        if (!empty($group_ids)) {
            ...
            // Cast to float to support bigint data type
            if ($return =
                $this->db->delete($this->tables['users_groups'], array($this->join['users'] => (float)$user_id))) {
    ...
    public function create_group($group_name = FALSE, $group_description = '', $additional_data = array()) {
        ...
        if (!empty($additional_data)) $data =
            array_merge($this->_filter_data($this->tables['groups'], $additional_data), $data);
