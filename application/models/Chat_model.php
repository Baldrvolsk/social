...
class Chat_model extends CI_Model
{
    ...
    public function get_user_chats() {
        $q = 'SELECT c.*, concat(f.first_name," ",f.last_name) as from_user, concat(t.first_name," ", t.last_name) as to_user
              FROM `chat` c 
              LEFT JOIN user f ON f.id = c.from 
              LEFT JOIN user t ON t.id = c.to 
              WHERE c.from = ' . $this->user->id . ' OR c.to = ' . $this->user->id;
    ...
    public function get_chat($id = 0) {
        $this->db->select('chat_message.*, user.first_name, user.last_name, user.avatar');
        $this->db->from($this->message_table);
        $this->db->join('user', 'user.id=chat_message.user_id', 'LEFT');
